    ...
    try:
        ...
        pass_payload = {
            ...
            'textModulesData': [
                {
                    'header': 'Total Amount',
                    'body': f"${receipt_data.get('total_amount', 0.0):.2f}",
                    'id': 'total'
                },
                {
                ...
                },
                {
                    ...
                    'body': "\n".join([
                        f"{item.get('quantity', 1)}x {item.get('description', 'Item')} - ${item.get('price', 0.0):.2f}"
...
def generate_wallet_pass_link(receipt_data: dict) -> str | None:
    """
